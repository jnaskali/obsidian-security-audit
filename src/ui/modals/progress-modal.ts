...
import { createSummaryTable, getMaxSeverity } from './utils';
...
export class ProgressModal extends Modal {
	...
	constructor(app: App, plugin: any, onCancel?: () => void) {
		super(app);
		this.plugin = plugin;
	...
	onOpen() {
		...
		contentEl.createEl('h3', {text: 'Running Security Audit'});

		this.statusText = contentEl.createEl('p', {text: 'Initializing...'});

		this.progressBar = contentEl.createEl('progress', { cls: 'progress-bar' });
		this.progressBar.max = 100;
		this.progressBar.value = 0;

		this.summaryContainer = contentEl.createEl('div');
		this.summaryContainer.style.display = 'none'; // Hidden initially

		this.buttonContainer = contentEl.createEl('div');
		this.buttonContainer.style.display = 'none'; // Hidden initially
	}

	updateProgress(value: number, status: string, showButton: boolean = false, buttonText: string = '', onButtonClick?: () => void, summary?: {critical: number, high: number, moderate: number, low: number, info: number, noIssues: number, failedDownload: number, auditIncomplete: number, noRepo: number}) {
		...
		if (summary) {
			...
			this.summaryContainer.createEl('h4', {text: 'Audit Summary'});
			const table = createSummaryTable(this.summaryContainer, 'Plugin category', 'Plugin count', summary);
		} else {
		...
		if (showButton && onButtonClick) {
			...
			const button = this.buttonContainer!.createEl('button', {text: buttonText});
	...
	private parseInsecurities(): string {
		...
		for (const section of sections) {
			...
			try {
				...
				const {maxSeverity, libraries} = getMaxSeverity(vulnerabilities);
				if (maxSeverity !== 'none') {
	...
	showReport(summary: {critical: number, high: number, moderate: number, low: number, info: number, noIssues: number, failedDownload: number, auditIncomplete: number, noRepo: number}, logContent: string) {
		...
		if (summary) {
			const table = createSummaryTable(contentEl, 'Plugin category', 'Plugin count', summary);
